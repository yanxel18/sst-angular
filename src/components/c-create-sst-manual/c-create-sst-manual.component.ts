...
export class CCreateSstManualComponent implements OnInit, OnDestroy {
  ...
  sstForm = new UntypedFormGroup({
    ...
    baseListCombo: new UntypedFormControl({ value: this.sstFormValue.baseID}, [Validators.required]), //remove  base value if toggle is enable
    localNumberTxt: new UntypedFormControl(''),
    /*
    localNumberTxt: new FormControl('', [Validators.required, Validators.minLength(6), LocalPhoneCharValidator()]),

        emailTxt: new FormControl(this.sstFormValue.emailTxt, [Validators.required, Validators.email]),
        applyUserName: new FormControl(this.sstFormValue.userFullName),
        baseListCombo: new FormControl({ value: this.sstFormValue.baseID}),
        localNumberTxt: new FormControl(this.sstFormValue.localNumberTxt),*/
  ...
  createSSTMSGBox(fdata: FormData): void {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.appSubscription.push(
          this.ccreateSSTService.postUploadFile(fdata).subscribe(response => {
            Swal.fire(
              '登録',
              'SSTを登録しました。',
              'success'
  ...
  loadDefault(): void {
    ...
    this.AccessRights = this.store.select(SSTSelectors.getUserInfo).pipe(
      map(t => t[0]?.su_access_rights![0]));
  ...
  loadtoValue(): void {
    ...
    this.appSubscription.push(
      this.ccreateSSTService.getActivityList().subscribe(data => {
        this.store.dispatch(SSTActions.LoadActivityList({ payload: data.filter( x => x.stc_id >1 && x.stg_active === 1) }));
      }, () => { }, () => {
        this.spinnerBox.LoadSuccess = false;
      })
    );
